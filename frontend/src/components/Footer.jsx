            ...
            <div className="container-x grid grid-cols-1 gap-12 py-16 sm:grid-cols-2 lg:grid-cols-4">
                ...
                <nav aria-label="Company">
                    ...
                    <ul className="mt-5 space-y-3">
                        {companyLinks.map((l) => (
                            <li key={l.to}>
                                <Link to={l.to} data-testid={`footer-company-${l.label.toLowerCase().replace(/[^a-z0-9]+/g, "-")}`} className="group inline-flex items-center gap-1 text-sm text-slate-400 transition-colors hover:text-white">
                                    {l.label}
                ...
                <nav aria-label="Solutions">
                    ...
                    <ul className="mt-5 space-y-3">
                        {SOLUTIONS.map((s) => (
                            <li key={s.slug}>
                                <Link to={`/solutions/${s.slug}`} data-testid={`footer-solution-${s.slug}`} className="group inline-flex items-center gap-1 text-sm text-slate-400 transition-colors hover:text-white">
                                    {s.title}
                                    <ArrowUpRight className="h-3.5 w-3.5 opacity-0 transition-opacity group-hover:opacity-100" aria-hidden="true" />
                                </Link>
                            </li>
            ...
            <div className="border-t border-white/10">
                <div className="container-x flex flex-col items-start justify-between gap-4 py-7 text-xs text-slate-500 sm:flex-row sm:items-center">
                    ...
                    <div className="flex gap-6">
                        <Link to="/privacy-policy" data-testid="footer-privacy-link" className="transition-colors hover:text-slate-300">Privacy Policy</Link>
                        <Link to="/terms-and-conditions" data-testid="footer-terms-link" className="transition-colors hover:text-slate-300">Terms &amp; Conditions</Link>
                    </div>
                </div>
            </div>
        </footer>
